...
### 💡 Word 导出特别说明（关于目录与页码生成）

为了保证生成的 Word 文档**绝对纯净且不触发任何 Office 安全警报**（例如“该文档包含的域可能引用了其他文件”的警告弹窗），本工具抛弃了老旧的强制刷新的“脏操作”，采用了微软官方推荐的**安全生成策略**。

当你下载并首次打开生成的 `.docx` 文档时，目录部分默认会是一行灰色的提示文字，而不是完整的目录。这完全是正常现象，请按照以下两步操作即可永久解决：

1. **一键计算**：打开文档后，按下键盘上的 **`Ctrl + A`**（全选），然后按下 **`F9`** 键（更新域），Word 会在瞬间利用其原生的排版引擎为你计算出最精准的页码并生成完美目录。
2. **永久生效**：按下 **`Ctrl + S`** 保存文档。

完成以上两步后，这个文档就被彻底“固化”了。此后无论你将它转发给谁、在任何设备或版本的 Office 中打开，它都会直接展示带有真实页码的目录，并且**永远不会弹出任何安全警告**！
